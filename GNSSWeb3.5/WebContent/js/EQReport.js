...
//	initReportChart();
	reportMap = new Map("reportMap", {logo:false,slider: true});  
	reportBaseLayer = new AMapTiledMapServiceLayer();
	reportMap.addLayer(reportBaseLayer, 0);
	reportMap.setScale(5000000);
	parent.drawStations(reportMap);
	centerAtEpi(reportMap);
	reportMap.on("click", reportMapClick);
	initReportEQLayer();
	initReportDetail();
	initReportChart();
...
function initReportDetail(){
	...
	$.ajax({
		...
		success:function(msg){
			...
			//alert(json.rail+"\n"+json.info.totalArea);
			//totalArea
			$("#area").text(json.info.totalArea + " 平方公里");
			//epi
			var epi="";
...
//					visibleEQLyrs.push(layers[i].id);
			...
			if(visibleEQLyrs == undefined){//visibleEQLyrs == undefined || visibleEQLyrs.length < 2
...
//			initReportMapCenter();
		...
		reportEQLayer.setVisibleLayers(visibleEQLyrs);
//		reportEQLyrDefinitions[lyrsMap["EQPoints"]] = "type='epicenter'";//EQPoints图层仅写入一个Epicenter和5个触发台站
		reportEQLyrDefinitions[lyrsMap["EQPolygons"]] = "num = 1";//EQPolygons图层每个台站写入一个Polygon对象，num值为1
		reportEQLayer.setLayerDefinitions(reportEQLyrDefinitions);
		reportEQLayer.refresh();
//		initReportMapCenter();
	}
...
function showReportPolygonResult(queryResult){
	...
	for(var i = 0; i < queryResult.features.length; i++){
        ...
        var attrInfos = {"EQID":eqID,"Lon":Lon,"Lat":Lat,"magnitude":attr.magnitude,"stationID":attr.stationID,"inTime":attr.inTime};
//    	var infoTemplate = new esri.InfoTemplate("${EQID}", "地震ID:${EQID}<br>经度:${Lon}<br>纬度:${Lat}<br>时报:${num}<br>震级:${magnitude}<br>所属台站:${stationID}<br><a href='staticCharts.html?stations=${stationID}&time=${inTime}&eqID=${EQID}'>查看数据</a>");
    	var infoTemplate = new esri.InfoTemplate("${EQID}", "地震ID:${EQID}<br>经度:${Lon}<br>纬度:${Lat}<br>震级:${magnitude}<br>所属台站:${stationID}");
