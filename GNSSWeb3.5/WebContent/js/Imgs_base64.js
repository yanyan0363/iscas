...
//		jsonp:'callback',
		success:function(msg){
			...
			for(var i=0;i<EQArrowsGif.length;i++,k++){
				...
				strNote+="<a href='data:application/octet-stream;base64,"+EQArrowsGif[i].content+"' download='"+name+".gif'>点击下载</a>";
				var imgNote = $("<div class='imgNote'>"+strNote+"</div>");
				imgDiv.append(imgNote);
				
				var img = $("<img id='"+name+"'>");
				img.attr("src","data:image/gif;base64,"+EQArrowsGif[i].content);
				img.attr("title",name);
				img.attr("width", "200px");
				img.attr("height", "200px");
				img.attr("onclick", "openImg(this.src)");
